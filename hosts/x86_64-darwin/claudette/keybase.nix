...

{
  launchd.user.agents."keybase".serviceConfig = rec {
    ProgramArguments = [
      "/bin/sh" "-c"
      "/bin/wait4path ${pkgs.keybase}/bin/keybase && exec ${pkgs.keybase}/bin/keybase -d service"
    ];
    ...
    ProgramArguments = [
      "/bin/sh" "-c"
      "/bin/wait4path ${pkgs.kbfs}/bin/kbfsfuse && exec ${pkgs.kbfs}/bin/kbfsfuse -mount-type=none"
    ];
    EnvironmentVariables = {
